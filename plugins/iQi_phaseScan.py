                ...
                for channelData in block["channelData"].values():
                    ...
                    if channelData["QIE"]:
                        ...
                        for (ts, adc) in enumerate(channelData["QIE"]):
                            ...
                            #        scan_bin = b - 1
                            #        break
                            scan_bin = (evt - EVT_START) / 100
                            # Ignore events which fall outside the bin range
                            #if scan_bin < 0 or (scan_bin > 49 and scan_bin < 64): continue
                            ...
                            """
                            if ts > 0:
                                book.fill((ts, adc), "NoTS0_ADC_vs_TS_phase_%d" % scan_bin,
                                      (7, nAdcMax), (0.5, -0.5), (7.5, nAdcMax-0.5),
                                      title="ADC vs TS  Phase scan index %d;time slice;ADC;Counts / bin" % scan_bin)
                                
                                book.fill((ts, charge), "NoTS0_Charge_vs_TS_phase_%d" % scan_bin,
                                      (7, 100), (0.5, -0.5), (7.5, 14999.5),
                                      title="Charge vs TS  Phase scan index %d;time slice;Charge [fC];Counts / bin" % scan_bin)
