...
def iQi_phaseScan(raw1={}, raw2={}, book=None, warnQuality=True, fewerHistos=False, **other):
    # sanity check
		...
		for channelData in block["channelData"].values():
			...
			for (ts, adc) in enumerate(channelData["QIE"]):
			    ...
			    charge = float(adcCharges[adc])
                            if slot != 2: continue
                            if fib not in SLOT2_FIBERS: continue
			    if ts in [1,2,3]:
                                book.fill((evt, charge), "TS_%d_Charge_vs_EvtNum_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d" % (ts, fedId, crate, slot, fib, fibCh), TOTAL_EVENTS, 0.5, TOTAL_EVENTS + 0.5, title="Charge vs Event Number  TS %d  FED %d Crate %d Slot %d Fib %d Ch %d;Event Number;Charge [fC];Counts / bin" % (ts, fedId, crate, slot, fib, fibCh))
                                book.fill((evt, tdc), "TS_%d_TDC_vs_EvtNum_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d" % (ts, fedId, crate, slot, fib, fibCh), TOTAL_EVENTS, 0.5, TOTAL_EVENTS + 0.5, title="TDC vs Event Number  TS %d  FED %d Crate %d Slot %d Fib %d Ch %d;Event Number;TDC;Counts / bin" % (ts, fedId, crate, slot, fib, fibCh))
