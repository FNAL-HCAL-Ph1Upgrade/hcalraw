...
#MAX_SETTINGS = 64
MAX_SETTINGS = 8
scan_bins = [1 + n * EVENTS_PER_SETTING for n in range(MAX_SETTINGS + 1)]
...
def iqiScan(raw1={}, raw2={}, book=None, warnQuality=True, fewerHistos=False, **other):
    # sanity check
                ...
                for channelData in block["channelData"].values():
                    ...
                    if channelData["QIE"]:
                        ...
                        for (ts, adc) in enumerate(channelData["QIE"]):
                            ...
                            scan_bin = -1
                            for b, lim in enumerate(scan_bins):
                                if evt < lim:
                                    scan_bin = b - 1
                                    break
                            ...
                            book.fill((scan_bin, adc),
                                  "ADC_iQi_scan_%s_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d_2D" % (errf, fedId, crate, slot, fib, fibCh),
                            ...
                            book.fill((scan_bin, charge),
                                  "Charge_iQi_scan_%s_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d_2D" % (errf, fedId, crate, slot, fib, fibCh),
                                  (MAX_SETTINGS, 100), (-0.5, -0.5), (MAX_SETTINGS - 0.5, 14999.5),
                            ...
                            if ts == 2:
                                book.fill((evt, adc), 
                                    "TS_2_ADC_vs_EvtNum_%s_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d_2D" % (errf, fedId, crate, slot, fib, fibCh), 
                                    (800, nAdcMax), (-0.5, -0.5), (799.5, nAdcMax - 0.5), 
                                    title="iQi Scan  ADC vs Evt Num  TS 2 FED %d Crate %d Slot %d Fib %d Ch %d (ErrF %s 0);Event number;ADC;Counts / bin" % (fedId, crate, slot, fib, fibCh, eq))
                                
                                book.fill((evt, charge), 
                                    "TS_2_Charge_vs_EvtNum_%s_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d_2D" % (errf, fedId, crate, slot, fib, fibCh), 
                                    (800, 100), (-0.5, -0.5), (799.5, 14999.5), 
                                    title="iQi Scan  Charge vs Evt Num  TS 2 FED %d Crate %d Slot %d Fib %d Ch %d (ErrF %s 0);Event number;Charge [fC];Counts / bin" % (fedId, crate, slot, fib, fibCh, eq))

                                book.fill((evt, charge), 
                                    "TS_2_Charge_vs_EvtNum", 
                                    (800, 100), (-0.5, -0.5), (799.5, 14999.5), 
                                    title="iQi Scan  Charge vs Evt Num  TS 2;Event number;Charge [fC];Counts / bin")
                                
                                book.fill((evt, charge), 
                                    "TS_2_Charge_vs_EvtNum_%s" % errf, 
                                    (800, 100), (-0.5, -0.5), (799.5, 14999.5), 
                                    title="iQi Scan  Charge vs Evt Num  TS 2 (ErrF %s 0);Event number;Charge [fC];Counts / bin" % eq)
                            #book.fill((scan_bin, charge),
