...
def pedestal(raw1={}, raw2={}, book=None, warnQuality=True, fewerHistos=False, **other):
    # sanity check
	    ...
	    #print "evt %d\tiEvent %d" % (evt, gEvt)
            ...
            for i, block in enumerate(blocks):
                ...
                for channelData in block["channelData"].values():
                    ...
                    if channelData["QIE"]:
                        ...
                        for (ts, adc) in enumerate(channelData["QIE"]):
                            ...
                            book.fill((ts, adc), "NoTS0_ADC_vs_TS",
                                (nTsMax, nAdcMax), (-0.5, -0.5), (nTsMax-0.5, nAdcMax-0.5),
                                title="ADC vs TS;time slice;ADC;Counts / bin")
			    book.fill((ts, adc), 
				"ADC_vs_TS_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d_2D" % (fedId, crate, slot, fib, fibCh), 
				(nTsMax, nAdcMax), (-0.5, -0.5), (nTsMax-0.5, nAdcMax - 0.5),
			    ...
			    """ 
                            if ts > 0:
                                book.fill((ts, adc), "NoTS0_ADC_vs_TS",
                                      (7, nAdcMax), (0.5, -0.5), (7.5, nAdcMax-0.5),
                                      title="ADC vs TS;time slice;ADC;Counts / bin")

                                book.fill((ts, charge), "NoTS0_Charge_vs_TS",
                                      (7, 20), (0.5, -0.5), (7.5, 99.5),
                                      title="Charge vs TS;time slice;Charge [fC];Counts / bin")

				book.fill((ts, adc), 
                                      "NoTS0_ADC_vs_TS_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d_2D" % (fedId, crate, slot, fib, fibCh), (7, nAdcMax), (0.5, -0.5), (7.5, nAdcMax - 0.5),
                                      title="ADC vs TS  FED %d Crate %d Slot %d Fib %d Ch %d;time slice;ADC;Counts / bin" % (fedId, crate, slot, fib, fibCh))

                                book.fill((ts, charge), "NoTS0_Charge_vs_TS_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d_2D" % (fedId, crate, slot, fib, fibCh),
                                      (7, 100), (0.5, -0.5), (7.5, 20000),
                                      title="Charge vs TS  FED %d Crate %d Slot %d Fib %d Ch %d;time slice;Charge [fC];Counts / bin" % (fedId, crate, slot, fib, fibCh))
			    """
