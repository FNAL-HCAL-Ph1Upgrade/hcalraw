...
def pedScan(raw1={}, raw2={}, book=None, warnQuality=True, fewerHistos=False, **other):
    # sanity check
    for r, raw in enumerate([raw1, raw2]):
		...
		#		    break
			    ...
			    if ts > 0:
				#book.fill((evt,adc), "ADC_vs_EvtNum_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d_1D" % (fedId, crate, slot, fib, fibCh), TOTAL_EVENTS, 0.5, TOTAL_EVENTS + 0.5, title="ADC vs Event Number  FED %d Crate %d Slot %d Fib %d Ch %d;Event number;ADC;Counts / bin" % (fedId, crate, slot, fib, fibCh))

				book.fill((evt,charge), "Charge_vs_EvtNum_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d" % (fedId, crate, slot, fib, fibCh), TOTAL_EVENTS, 0.5, TOTAL_EVENTS + 0.5, title="Charge vs Event Number  FED %d Crate %d Slot %d Fib %d Ch %d;Event number;Charge [fC];Counts / bin" % (fedId, crate, slot, fib, fibCh))
