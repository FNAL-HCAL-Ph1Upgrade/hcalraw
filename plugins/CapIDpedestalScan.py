...
def CapIDpedestalScan(raw1={}, raw2={}, book=None, warnQuality=True, fewerHistos=False, **other):
    # sanity check
	...
	for fedId, dct in sorted(raw.iteritems()):
		...
		for channelData in block["channelData"].values():
			...
			#	    break
			    ...
			    #book.fill((evt, adc), "TS_%d_ADC_vs_EvtNum_2D" % ts, (TOTAL_EVENTS, nAdcMax), (0.5, -0.5), (TOTAL_EVENTS + 0.5, nAdcMax - 0.5), title="ADC vs Event Number  TS %d;Event number;ADC;Counts / bin" % ts)
			    #book.fill((evt, adc), "TS_%d_ADC_vs_EvtNum_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d_2D" % (ts, fedId, crate, slot, fib, fibCh), (TOTAL_EVENTS, nAdcMax), (0.5, -0.5), (TOTAL_EVENTS + 0.5, nAdcMax - 0.5), title="ADC vs Event Number  TS %d FED %d Crate %d Slot %d Fib %d Ch %d;Event number;ADC;Counts / bin" % (ts, fedId, crate, slot, fib, fibCh))
			    if ts in xrange(1,5):
				book.fill((evt, charge), "CapID_%d_Charge_vs_EvtNum_FED_%d_Crate_%d_Slot_%d_Fib_%d_Ch_%d" % (ts%4, fedId, crate, slot, fib, fibCh), TOTAL_EVENTS, 0.5, TOTAL_EVENTS + 0.5, title="Charge vs Event Number  CapID %d FED %d Crate %d Slot %d Fib %d Ch %d;Event number;Charge [fC];Counts / bin" % (ts%4, fedId, crate, slot, fib, fibCh)) 	    
